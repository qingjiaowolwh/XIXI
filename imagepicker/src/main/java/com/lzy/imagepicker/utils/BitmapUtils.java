...
public class BitmapUtils {
     ...
     * @param rect
     */
    public static Bitmap changeSize(String path,Rect rect) {
        Bitmap saveBitmap = null;
        Bitmap bitmap = null;
        try {
            bitmap = BitmapFactory.decodeFile(path); // 此时返回bm为空
            saveBitmap = Bitmap.createBitmap(bitmap,rect.left-rect.width()/2,rect.top-rect.height()/2,rect.width()*2,rect.height()*2);
        } catch (Exception e){
            return bitmap;
        }
        return saveBitmap;
